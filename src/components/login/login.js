...
import { getLedgerTransportMethod, getLedgerAccount } from '../../utils/ledger';
...
class Login extends React.Component {
  ...
  ledgerLogin() {
    ...
    const account = getLedgerAccount();
    let liskLedger;

    if (!transportMethod) {
      this.props.errorToastDisplayed({ label: i18next.t('Unable to connect to Ledger Nano: Correct Transport Layer not detected') });
      return;
    }
    ...
    transportMethod.create()
      ...
      .then(() => {
        liskLedger.getPubKey(account)
          ...
          .catch(() => {
            this.props.errorToastDisplayed({ label: i18next.t('Unable to connect to Ledger Nano S. Be sure your device is unlocked and inside the Lisk App') });
          });
      });
  }
